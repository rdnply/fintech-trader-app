...
const robotCreateFields = "owner_user_id, is_favorite, is_active"
...
func (s *RobotStorage) Create(r *robot.Robot) error {
	if err := s.createStmt.QueryRow(r.OwnerUserID, r.IsFavorite, r.IsActive).Scan(&r.RobotID); err != nil {
...
const robotFields =
	"owner_user_id, parent_user_id, is_favorite, is_active, ticker, buy_price, sell_price, plan_start, plan_end, " +
...
const findRobotByTickerQuery = "SELECT robot_id, " + robotFields + " FROM users WHERE ticker=$1"
...
const updateRobotQuery =
	"UPDATE robots SET " +
	"owner_user_id=$2, parent_user_id=$3, is_favorite=$4, is_active=$5, ticker=$6, buy_price=$7, " +
...
func robotArgs(r *robot.Robot) []interface{} {
	return []interface{}{&r.RobotID, &r.OwnerUserID, &r.ParentRobotID, &r.IsFavorite, &r.IsActive, &r.Ticker, &r.BuyPrice, &r.SellPrice,
